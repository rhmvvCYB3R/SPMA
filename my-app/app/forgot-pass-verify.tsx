...
import { useEffect, useRef, useState } from 'react'; // Добавил useEffect
import {
...
export default function ForgotPassVerificationScreen() {
  ...
  const [timer, setTimer] = useState(0); // Состояние таймера
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (flow === 'forgot-password') {
        ...
        router.replace({ 
          pathname: '/forgot-pass-change-password', 
          params: { 
            tempCode: fullCode,
            email: email 
          } 
...
const styles = StyleSheet.create({
  ...
  errorBox: { backgroundColor: '#FFEBEE', padding: spacing.sm, borderRadius: 8, marginBottom: spacing.md, width: '100%' },
  errorText: { color: '#D32F2F', fontSize: fontSize.sm, textAlign: 'center', fontWeight: '600' },
  successBox: { backgroundColor: '#E8F5E9', padding: spacing.sm, borderRadius: 8, marginBottom: spacing.md, width: '100%' },
